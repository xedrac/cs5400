...
Modified by Shawn Badger
    2011-06-06 - Fixed critical bugs causing parsing to fail
*/
...
)
{
  ...
  for (i = 0; i < nprops; i++) {
    ...
    prop->external_type  = prop->external_type;
...
)
{
  ...
  prop_ptr->external_type  = prop->external_type;
  prop_ptr->internal_type  = prop->internal_type;
  prop_ptr->offset         = prop->offset;
  prop_ptr->count_external = prop->count_external;
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list) {       /* a list */
      ...
      if (list_count == 0) {
        ...
        for (k = 0; k < list_count; k++) {
          get_ascii_item (words[which_word++], prop->internal_type, &int_val, &uint_val, &double_val);
          if (store_it) {
...
)
{
  switch (type) {
    ...
    default:
      fprintf (stderr, "get_ascii_item: bad type = %d (word: %s)\n", type, word);
      exit (-1);
  }
}
